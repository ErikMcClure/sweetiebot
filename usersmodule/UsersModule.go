...
func (w *UsersModule) OnGuildMemberAdd(info *bot.GuildInfo, m *discordgo.Member, t time.Time) {
	if info.Config.Users.NotifyChannel != bot.ChannelEmpty {
		created := "(Created " + bot.TimeDiff(t.Sub(bot.SnowflakeTime(bot.SBatoi(m.User.ID)))) + " ago)"
		info.SendMessage(info.Config.Users.NotifyChannel, "<@"+m.User.ID+"> "+created+" joined.")
	}
}
