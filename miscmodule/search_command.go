...
func (c *searchCommand) Process(args []string, msg *discordgo.Message, indices []int, info *bot.GuildInfo) (string, bool, *discordgo.MessageEmbed) {
	...
	for _, v := range users {
		...
		if bot.UserRegex.MatchString(v) {
			...
			IDs := info.FindUsername(v[1:], false)
