...
func (w *SpoilerModule) Channels() []string {
  return []string{"manechat", "mylittleactivities", "mylittleroleplay", "mylittleponyfinder", "mylittlecoders", "mylittlebot", "bot-debug" }
}
...
func (w *SpoilerModule) HasSpoiler(s *discordgo.Session, m *discordgo.Message) bool {
  if w.spoilerban != nil && w.spoilerban.MatchString(strings.ToLower(m.Content)) {
    ...
    if RateLimit(&w.lastmsg, sb.config.Maxspoiltime) {
      s.ChannelMessageSend(m.ChannelID, "[](/sbtarget) ```POSTING SPOILERS IS A BANNABLE OFFENSE. All discussion about future episodes or seasons MUST be in #mylittlespoilers.```")
    }
    return true
  }
  return false
...
func (c *AddSpoilerCommand) Process(args []string, user *discordgo.User) (string, bool) {  
  if len(args) < 1 {
