...
func (w *WittyModule) OnMessageCreate(info *GuildInfo, m *discordgo.Message) {
	...
	/*if w.shutupregex.MatchString(str) {
	  if CheckRateLimit(&info.lastshutup, info.config.Maxshutup) {
	    info.SendMessage(m.ChannelID, "[](/sadbot) `Sorry! (All comments and public commands disabled in #example for the next " + TimeDiff(time.Duration(info.config.Maxshutup) * time.Second) + ").`")
	  }
