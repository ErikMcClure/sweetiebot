...
func (c *ImportCommand) Usage(info *GuildInfo) *CommandUsage {
	return &CommandUsage{
		Desc: "Adds all elements from the source collection on the source server to the target collection on this server. If no target is specified, attempts to copy all items into a collection of the same name as the source. Example: ```" + info.config.Basic.CommandPrefix + "import ExampleServer cool notcool```",
		Params: []CommandUsageParam{
